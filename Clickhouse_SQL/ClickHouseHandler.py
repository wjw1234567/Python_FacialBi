...
class ClickHouseHandler:
    ...
    def stream_query_insert(self, source_sql: str, target_table: str, batch_size: int = 10000):
        """
        ...
        with Client(host=self.host, port=self.port, user=self.user,
            ...
            # 获取查询列名
            column_names = self._get_query_columns(source_sql, client)
            print(f"使用列名: {column_names}")

            batch = []
            for row in client.execute_iter(source_sql):
                batch.append(row)
    ...
    def _get_query_columns(self, sql: str, client: Client):
        """
        获取 SQL 查询列名
        """
        # LIMIT 0 获取列信息，不拉取数据
        res = client.execute(f"SELECT * FROM ({sql}) LIMIT 0", with_column_types=True)
        columns = [col[0] for col in res[1]]
        return columns

    def delete_partition(self, table_name: str, delete_sql: str):
        """
        删除目标表分区数据
        """
        with Client(host=self.host, port=self.port, user=self.user,
...
if __name__ == "__main__":
    ...
    ch.delete_partition(target_table,delete_sql)
